...
function getMeTokens(req, res) {
  try {
    return res.status(200).json(getTokensState(req.user.id));
  } catch (err) {
...
function creditMeTokens(req, res) {
  const amount = Number(req.body?.amount);
  if (!Number.isFinite(amount) || amount <= 0 || amount > 100000) {
    return res.status(400).json({ message: 'amount must be a positive number' });
  }
  try {
    const state = creditTokens(req.user.id, amount);
